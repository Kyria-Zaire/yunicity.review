#!/usr/bin/env python3
"""Backfill passport challenge progress from historical stamps and redemptions (PASSPORT-04B).

Usage (from backend/):
    python scripts/backfill_passport_challenges.py --stamps --dry-run
    python scripts/backfill_passport_challenges.py --stamps --execute --limit 100
    python scripts/backfill_passport_challenges.py --stamps --redemptions --execute
"""
...
from app.services.passport_challenge_backfill_service import (  # noqa: E402
    BackfillReport,
    PassportChallengeBackfillService,
)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Backfill progression défis Passport depuis tampons et redemptions historiques."
        ),
    )
    parser.add_argument(
        "--stamps",
    ...
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Créer progression défis manquante.",
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def _print_report(label: str, report: BackfillReport) -> None:
    ...
    print(f"progressed        : {report.progressed}")
