#!/usr/bin/env python3
"""Backfill YuniMonnaie from historical stamps and redemptions (PASSPORT-02B).

Usage (from backend/):
    python scripts/backfill_yuni_wallet.py --stamps --dry-run
    python scripts/backfill_yuni_wallet.py --stamps --execute --limit 100
    python scripts/backfill_yuni_wallet.py --stamps --redemptions --execute
"""
...
from app.services.yuni_wallet_backfill_service import (  # noqa: E402
    YuniBackfillReport,
    YuniWalletBackfillService,
)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Backfill YuniMonnaie depuis tampons et redemptions historiques.",
    )
    parser.add_argument(
        "--stamps",
        action="store_true",
    ...
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Créer wallets et transactions YM manquants.",
    )
    parser.add_argument(
        "--limit",
        type=int,
...
def _print_report(label: str, report: YuniBackfillReport) -> None:
    print(f"\n=== {label} ===")
    ...
    print(f"skipped_suspended : {report.skipped_suspended}")
