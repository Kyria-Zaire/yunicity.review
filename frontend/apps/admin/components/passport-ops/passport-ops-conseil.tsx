import type { PassportOpsRecommendedAction } from "@yunicity/utils";
import { GraduationCap } from "lucide-react";
import Link from "next/link";

type PassportOpsConseilProps = {
  message: string;
  action: PassportOpsRecommendedAction;
};

export function PassportOpsConseil({ message, action }: PassportOpsConseilProps) {
  return (
    <section
      ...
      aria-labelledby="passport-ops-conseil-title"
    >
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          <h2
            id="passport-ops-conseil-title"
            className="text-sm font-bold text-stone-950"
          >
            Conseil
          </h2>
          <p className="mt-0.5 text-sm text-stone-700">
            Le Passport est le cœur de l&apos;engagement citoyen. {message}
          </p>
        </div>
      </div>
      <Link
