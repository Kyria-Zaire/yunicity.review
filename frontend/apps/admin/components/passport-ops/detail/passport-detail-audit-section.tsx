...
interface PassportDetailAuditSectionProps {
...
function actorLabel(item: AdminPassportActionItem): string {
...
export function PassportDetailAuditSection({
  items,
  total,
  page,
  pageSize,
  ...
  onRetry,
  onPageChange,
}: PassportDetailAuditSectionProps) {
  return (
                  ...
                  {items.map((item) => (
                    <tr key={item.id} className="hover:bg-stone-50/80">
                      <td className="px-4 py-3 font-medium text-stone-900">
                        {adminPassportActionLabel(item.action)}
                      </td>
