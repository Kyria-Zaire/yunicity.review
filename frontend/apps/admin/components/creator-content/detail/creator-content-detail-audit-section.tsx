...
interface CreatorContentDetailAuditSectionProps {
...
function actorLabel(item: AdminCreatorContentActionItem): string {
...
export function CreatorContentDetailAuditSection({
  items,
  total,
  page,
  pageSize,
  ...
  onRetry,
  onPageChange,
}: CreatorContentDetailAuditSectionProps) {
  return (
                  ...
                  {items.map((item) => (
                    <tr key={item.id} className="hover:bg-stone-50/80">
                      <td className="px-4 py-3 font-medium text-stone-900">
                        {creatorContentAdminActionLabel(item.action)}
                      </td>
                      ...
                      <td className="px-4 py-3 text-xs text-stone-500">
                        {formatCreatorContentDate(item.created_at)}
                      </td>
                    </tr>
                  ))}
                </tbody>
