...
interface CreatorContentRejectDialogProps {
  contentTitle: string;
  isOpen: boolean;
...
export function CreatorContentRejectDialog({
  contentTitle,
  isOpen,
  isSubmitting,
  onClose,
  onConfirm,
}: CreatorContentRejectDialogProps) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="reject-creator-content-title"
    >
      <form
        onSubmit={handleSubmit}
        className="w-full max-w-md rounded-2xl border border-stone-200 bg-white p-6 shadow-xl"
      >
        <h3 id="reject-creator-content-title" className="text-lg font-semibold text-stone-900">
          Refuser le contenu
        </h3>
        <p className="mt-2 text-sm text-stone-600">
          <span className="font-medium text-stone-800">{contentTitle}</span> — le partenaire pourra
