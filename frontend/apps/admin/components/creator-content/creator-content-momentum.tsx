import type { CreatorContentEditorialMomentum } from "@yunicity/utils";
import { formatAdminMetric } from "@yunicity/utils";

export function CreatorContentMomentum({ momentum }: { momentum: CreatorContentEditorialMomentum }) {
  return (
    <section
      className="rounded-xl border border-stone-200 bg-white px-4 py-3 shadow-sm"
      aria-labelledby="creator-content-momentum-title"
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="min-w-0 shrink-0 sm:w-56">
          <h2
            id="creator-content-momentum-title"
            className="text-[10px] font-semibold uppercase tracking-[0.18em] text-stone-500"
          >
            Objectif pilote éditorial
          </h2>
          <p className="mt-0.5 text-sm font-medium text-stone-800">
            {formatAdminMetric(momentum.approvedCount)} / {momentum.goal} contenus approuvés
          </p>
        </div>
        ...
        <div className="min-w-0 flex-1">
          <div
            ...
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`Progression éditoriale : ${momentum.progressPercent} pourcent`}
          >
            <div
