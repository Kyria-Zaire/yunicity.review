import type { PartnerLeadPilotMomentum } from "@yunicity/utils";
import { formatAdminMetric } from "@yunicity/utils";
import { TrendingUp } from "lucide-react";

export function PartnerLeadsPilotMomentum({ momentum }: { momentum: PartnerLeadPilotMomentum }) {
  return (
    <section
      className="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-5"
      aria-labelledby="partner-leads-pilot-momentum-title"
    >
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h2
            id="partner-leads-pilot-momentum-title"
            className="mt-1 text-base font-bold tracking-tight text-stone-950 sm:text-lg"
          >
            {momentum.title}
          </h2>
          <p className="mt-1 text-sm text-stone-600">{momentum.description}</p>

          <div className="mt-4 flex flex-wrap items-end justify-between gap-2">
            <p className="text-sm font-semibold text-stone-900">
              {formatAdminMetric(momentum.qualifiedCount)} / {momentum.goal} prospects qualifiés
            </p>
          ...
          <p className="mt-2 text-sm text-stone-600">{momentum.microcopy}</p>
        </div>
      </div>
    </section>
  );
