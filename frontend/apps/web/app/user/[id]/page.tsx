...
export default function UserProfilePage() {
    ...
    async function load() {
      ...
      try {
        const data = isAuthenticated
          ? await yunicityApi.getPublicProfileByUserId(userId)
          : await fetchPublicProfileByUserIdAnonymous(getWebApiBaseUrl(), userId);
        if (!cancelled) {
          setProfile(data);
        }
  ...
  }, [userId, isAuthenticated, yunicityApi]);
    ...
    <div className="min-h-screen bg-yunicity-background">
      <header className="border-b border-neutral-200/80 bg-white px-4 py-4">
        <div className="mx-auto flex max-w-lg items-center justify-between">
          <Link href="/feed" className="font-bold text-neutral-900">
            Yunicity
          </Link>
          {isAuthenticated ? (
          ...
          <article className="space-y-6">
            <section className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
              <header className="flex items-center gap-4">
                <ProfileAvatar name={profile.display_name ?? profile.username} size="lg" />
                <div>
              ...
              {profile.bio ? (
                <p className="mt-4 leading-relaxed text-neutral-700">{profile.bio}</p>
              ) : null}
