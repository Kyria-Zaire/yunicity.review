...
import { PLACE_DETAIL_MEDIUM_GALLERY } from "@yunicity/utils";
import { Camera, ChevronLeft, ChevronRight, X } from "lucide-react";
import { useCallback, useEffect, useState } from "react";
import { createPortal } from "react-dom";

type PlaceMediumDetailGalleryProps = {
  title: string;
  imageUrls: string[];
  onRegisterOpen?: (open: (index?: number) => void) => void;
};

/** Galerie medium : 1 photo large + 2 vignettes (MEDIUM-LIEUX-DETAIL-01). */
export function PlaceMediumDetailGallery({
  title,
  imageUrls,
  onRegisterOpen,
}: PlaceMediumDetailGalleryProps) {
    ...
    lightboxOpen && mounted && imageUrls.length > 0
      ? createPortal(
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-label={PLACE_DETAIL_MEDIUM_GALLERY(imageUrls.length)}
            onClick={close}
          >
            <button
              type="button"
  ...
  return (
    <>
      <div className="space-y-2" data-place-medium-detail-gallery="">
        <button
          type="button"
        ...
        <div className="grid grid-cols-2 gap-2">
          {paddedSecondary.map((url, index) => (
            <button
              ...
              className="relative min-h-[120px] overflow-hidden rounded-2xl bg-neutral-200 sm:min-h-[140px]"
            >
              <CulturalImage
                src={url}
                alt=""
              ...
              {index === 1 && imageUrls.length > 0 ? (
                <span
                  className="absolute bottom-3 right-3 inline-flex items-center gap-2 rounded-full bg-white/95 px-3 py-1.5 text-xs font-semibold text-neutral-800 shadow-sm"
                  onClick={(event) => {
                ...
                >
                  ...
                  {PLACE_DETAIL_MEDIUM_GALLERY(imageUrls.length)}
                </span>
              ) : null}
            </button>
          ))}
