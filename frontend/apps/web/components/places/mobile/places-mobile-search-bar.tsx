...
import {
  PLACES_MOBILE_FILTERS_ARIA,
  PLACES_MOBILE_FILTERS_SOON,
  PLACES_MOBILE_SEARCH_PLACEHOLDER,
} from "@yunicity/utils";
import { Search, SlidersHorizontal } from "lucide-react";
import { useRouter } from "next/navigation";

type PlacesMobileSearchBarProps = {
  query: string;
  onQueryChange: (value: string) => void;
};

/** Recherche + filtre mobile Lieux (MOBILE-LIEUX-01). */
export function PlacesMobileSearchBar({ query, onQueryChange }: PlacesMobileSearchBarProps) {
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    if (!trimmed) {
      router.push("/search?group=places");
      return;
    }
    router.push(`/search?q=${encodeURIComponent(trimmed)}&group=places`);
  }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        type="button"
        disabled
        title={PLACES_MOBILE_FILTERS_SOON}
        aria-label={PLACES_MOBILE_FILTERS_ARIA}
