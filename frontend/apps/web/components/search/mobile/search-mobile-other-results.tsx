...
import { SEARCH_DESKTOP_OTHER_RESULTS, SEARCH_MOBILE_MORE_BELOW } from "@yunicity/utils";
import {
  Briefcase,
  Building2,
  CalendarDays,
...
type SearchMobileOtherResultsProps = {
  rows: Array<{
    id: string;
...
export function SearchMobileOtherResults({ rows, onSelect }: SearchMobileOtherResultsProps) {
  if (rows.length === 0) return null;

  return (
    <section className="space-y-3" aria-labelledby="search-mobile-other-title">
      <h2 id="search-mobile-other-title" className="text-base font-bold text-neutral-950">
          ...
          return (
            <li key={row.id}>
              <button
                ...
                className="flex w-full items-center gap-3 rounded-2xl border border-neutral-200/90 bg-white px-4 py-3 text-left shadow-sm transition hover:border-neutral-300"
              >
                <span
      ...
      <p className="flex items-center justify-center gap-1 pt-2 text-center text-xs text-neutral-400">
