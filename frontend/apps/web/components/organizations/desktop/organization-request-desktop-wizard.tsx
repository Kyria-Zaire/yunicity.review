...
type OrganizationRequestDesktopWizardProps = {
  ...
  return (
    <span
      className={`pointer-events-none absolute right-3 text-xs tabular-nums text-neutral-400 ${
        placement === "input" ? "top-1/2 -translate-y-1/2" : "bottom-3"
      }`}
    >
      {value} / {max}
    </span>
...
export function OrganizationRequestDesktopWizard({
  step,
  draft,
  neighborhoods,
  selectedCategory,
  ...
  onSaveDraft,
  onSubmit,
}: OrganizationRequestDesktopWizardProps) {
  const { user } = useAuth();
  ...
  const nextLabel =
    step === "verification" ? ORG_REQUEST_SUBMIT : organizationRequestNextStepLabel(step);

  return (
    <div
      className="rounded-2xl border border-neutral-200/90 bg-white p-5 shadow-sm sm:p-6"
      data-org-request-desktop-wizard=""
    >
      {validationMessage ? (
      ...
      {step === "identity" ? (
        <div>
          <h2 className="text-xl font-bold text-neutral-900">{ORG_REQUEST_IDENTITY_TITLE}</h2>
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="block space-y-2">
                ...
                <div className="relative">
                  ...
                  />
                  <CharCounter value={nameCount} max={ORG_REQUEST_NAME_MAX} placement="input" />
                </div>
              </label>
                ...
                {ORGANIZATION_REQUEST_CATEGORY_OPTIONS.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex min-h-[5.5rem] flex-col items-start justify-center gap-2 rounded-xl border px-3 py-3 text-left transition ${
                        selected
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <label className="block space-y-2">
                ...
                <div className="relative">
                  ...
                  />
                  <CharCounter value={shortDescCount} max={ORG_REQUEST_SHORT_DESC_MAX} />
                </div>
              </label>
            </div>
      ...
      {step === "address" ? (
        <div>
          <h2 className="text-xl font-bold text-neutral-900">{ORG_REQUEST_ADDRESS_TITLE}</h2>
      ...
      <div className="mt-6 flex flex-row flex-nowrap items-center justify-between gap-3 border-t border-neutral-100 pt-5">
        {step === "identity" ? (
          <Link
            href="/places"
        ...
        <div className="flex min-w-0 flex-row flex-nowrap items-center justify-end gap-2">
          <button
            type="button"
            onClick={onSaveDraft}
            className="shrink-0 rounded-xl border border-yunicity-primary bg-white px-3.5 py-2.5 text-sm font-semibold text-yunicity-primary transition hover:bg-[#EEF0FF]"
          >
            {ORG_REQUEST_SAVE_DRAFT}
          </button>
          {step === "verification" ? (
            <button
              ...
              className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-yunicity-primary px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-yunicity-primary-hover disabled:opacity-60"
            >
            ...
            <button
              type="button"
              onClick={onNext}
              className="inline-flex min-w-0 items-center gap-2 rounded-xl bg-yunicity-primary px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-yunicity-primary-hover"
            >
