...
export function VideosDiscoveryScreen({
  ...
  const mobileStates = (
    <>
      {isLoading ? (
        ...
        <>
          <VideosMobileFeaturedHero items={portal.featuredItems} />
          ...
          {portal.hasMore || hasMoreFeed ? (
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-full border border-neutral-200/90 bg-white py-3 text-sm font-semibold text-yunicity-primary shadow-sm disabled:opacity-60"
            >
              {VIDEOS_PORTAL_LOAD_MORE}
          ...
          <VideosMobileSubscriptionsRail creators={portal.creators} />
        </>
      )}
    </>
  );
  ...
  return (
    <>
      <div className="web-mobile-videos-only min-w-0 space-y-4 px-4 pb-2 pt-1">
        <VideosMobileHeader />
        <VideosMobileSearch
          value={portal.searchQuery}
          onChange={portal.setSearchQuery}
          filterOpen={filterOpen}
          onToggleFilter={() => setFilterOpen((open) => !open)}
        />
        <VideosMobileViewPills activeTab={portal.tab} onTabChange={portal.setTab} />
        {filterOpen ? (
      ...
      <div className="web-desktop-videos-only mx-auto w-full max-w-[1400px] px-3 py-2 sm:px-4 sm:py-4">
        ...
        <div className="grid gap-6 lg:grid-cols-[15rem_minmax(0,1fr)] lg:gap-8 xl:grid-cols-[16rem_minmax(0,1fr)]">
          ...
          <div className="min-w-0 space-y-8">{desktopStates}</div>
        </div>
      </div>
    </>
  );
