...
import { NotificationsDesktopRightRail } from "@/components/notifications/desktop/notifications-desktop-right-rail";
...
type NotificationsDesktopScreenProps = {
...
export function NotificationsDesktopScreen({
  allItems,
  unreadCount,
  preferences,
  isLoading,
  ...
  onPreferenceChange,
  onReload,
}: NotificationsDesktopScreenProps) {
  ...
  const filteredItems = useMemo(
    () => sortNotificationsForDesktop(filterNotificationsForDesktop(allItems, primaryTab, typeFilter), sort),
  ...
  const displayedCount = useMemo(() => countNotificationDesktopDisplayedRecent(allItems), [allItems]);

  const highlights = useMemo(
    () =>
      buildNotificationDesktopHighlights(allItems, (item) =>
        resolveNotificationDeeplink(item.deeplink, "web"),
      ),
    [allItems],
  );
  ...
  return (
    <div className="notifications-desktop-root w-full min-w-0" data-notifications-desktop-root="">
      <div className="grid w-full gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(280px,320px)] lg:gap-8">
