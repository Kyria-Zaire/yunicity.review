...
import { formatChallengeProgressPercent, getChallengeMotivationMessage } from "@yunicity/utils";
...
export function PassportChallengeCard({
  ...
  const motivation = getChallengeMotivationMessage(
    challenge.progress,
    challenge.target,
    challenge.completed,
  );
    ...
    <article className="rounded-2xl border border-neutral-200/90 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-sm font-medium text-yunicity-primary">{motivation}</p>
        </div>
