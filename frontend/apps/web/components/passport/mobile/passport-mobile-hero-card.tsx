...
export function PassportMobileHeroCard({
  ...
  return (
    <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-[#0f1f4d] via-[#152a66] to-[#1a3278] p-4 text-white shadow-lg">
      <div className="flex items-start gap-3">
          ...
          <div className="flex items-start gap-3">
            {avatarUrl ? (
              ...
              <img
                src={avatarUrl}
                alt={displayName}
                className="h-14 w-14 shrink-0 rounded-full border-2 border-white/30 object-cover"
              />
            ) : (
            ...
            <div className="min-w-0">
              ...
              {isActive ? (
                <span className="mt-1 inline-flex rounded-full bg-white/15 px-2.5 py-0.5 text-[11px] font-semibold text-[#C7D2FE]">
                  {PASSPORT_MOBILE_STATUS_ACTIVE}
                </span>
              ) : null}
              {memberSince ? (
                <p className="mt-1 text-xs text-white/70">{memberSince}</p>
              ) : null}
            </div>
          </div>

          <div className="mt-4">
            <div className="flex items-center gap-1.5 text-sm font-semibold text-[#C7D2FE]">
              <Star className="h-4 w-4 fill-[#FDE68A] text-[#FDE68A]" aria-hidden />
        ...
        <div className="flex shrink-0 flex-col items-center gap-1.5">
          ...
          <p className="text-[10px] font-medium text-white/80">{PASSPORT_MOBILE_QR_LABEL}</p>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-4 gap-2 border-t border-white/10 pt-4">
        {stats.map((stat) => (
          <div key={stat.label} className="text-center">
            ...
            <p className="mt-0.5 text-[10px] text-white/65">{stat.label}</p>
          </div>
        ))}
      </div>
    </section>
